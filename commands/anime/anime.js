...
module.exports = {
    ...
    callback: (client, interaction) => {
        let option = {
            uri: `https://kitsu.io/api/edge/anime/?filter[text]=${interaction.options.getString('title')}`,
            method: 'GET',
            headers: {
        ...
        get(option).then(
            async (mat) => {
                ...
                if (!dataApi){
                    interaction.reply('An error occurred while trying to find the anime. Maybe try another one?')
                    return
                }
